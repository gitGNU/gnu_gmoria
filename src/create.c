...
choose_race ()
{
  ...
  do
    {
      snprintf (tmp_str, sizeof (tmp_str), "%c) %s", k + 'a', race[j].trace);
  ...
  do
	...
	helpfile (PKGDATADIR "/welcome.hlp");
      else
	bell ();
    }
  while (!exit_flag);
...
get_history ()
{
  ...
  hist_ptr = background_start[py.misc.prace];
  history_block[0] = '\0';
  social_class = randint (4);
  cur_ptr = 0;
  size_t left = sizeof (history_block);
  int bail = 0;
  do
    {
      flag = FALSE;
      do
	{
	  if (background[cur_ptr].chart == hist_ptr)
		...
		cur_ptr++;
	      ...
	      size_t info_len = strlen (b_ptr->info);
	      if (info_len > left)
		{
		  bail = 1;
		  break;
		}
	      strncat (history_block, b_ptr->info, left);
	      left -= info_len;
...
get_class ()
{
  ...
  do
	...
	{
	  snprintf (tmp_str, sizeof (tmp_str), "%c) %s",
		    k + 'a', class[j].title);
	  put_buffer (tmp_str, m, l);
	  cl[k] = j;
	  l += 15;
  ...
  do
      ...
      if ((j < k) && (j >= 0))
	{
	  ...
	  m_ptr->bthb += c_ptr->mbthb;
	   /*RAK*/ m_ptr->srh += c_ptr->msrh;
...
get_money ()
{
  ...
  tmp = monval (a_ptr[A_STR]) + monval (a_ptr[A_INT])
    + monval (a_ptr[A_WIS]) + monval (a_ptr[A_CON]) + monval (a_ptr[A_DEX]);
...
create_character ()
{
  ...
  clear_from (20);
  put_buffer ("Hit space to reroll or ESC to accept characteristics: ", 20,
	      2);
  do
    {
      move_cursor (20, 56);
