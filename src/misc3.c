/* src/misc3.c: misc code for maintaining the dungeon, printing player info
  ...
  if (level == 0)
      ...
      do
	{
	  if (randint (2) == 1)
		...
		i =
		  randint (t_level[j] - t_level[j - 1]) - 1 + t_level[j - 1];
	    }
	}
      while ((must_be_small)
...
void
cnv_stat (stat, out_val, out_val_len)
     int8u stat;
     char *out_val;
     size_t out_val_len;
{
  ...
  if (stat > 18)
    {
      ...
      part2 = stat - 18;
      if (part2 == 100)
	snprintf (out_val, out_val_len, "18/100");
      else
	snprintf (out_val, out_val_len, " %2d/%02d", part1, part2);
    }
  else
    snprintf (out_val, out_val_len, "%6d", stat);
}
...
static void
prt_7lnum (header, num, row, column)
     char *header;
     int32 num;
     int row, column;
{
  ...
  snprintf (out_val, sizeof (out_val), "%s: %7ld", header, num);
  put_buffer (out_val, row, column);
}
     ...
     int row, column;
{
  ...
  snprintf (out_val, sizeof (out_val), "%6ld", num);
  put_buffer (out_val, row, column);
}
     ...
     int num, row, column;
{
  ...
  snprintf (out_val, sizeof (out_val), "%6d", num);
  put_buffer (out_val, row, column);
}
...
title_string ()
{
  ...
  if (py.misc.lev < 1)
    ...
    p = player_title[py.misc.pclass][py.misc.male][py.misc.lev - 1];
...
prt_depth ()
{
  ...
  depth = dun_level * 50;
  if (depth == 0)
    snprintf (depths, sizeof (depths), "%s", "Town level");
  else
    snprintf (depths, sizeof (depths), "%d feet", depth);
  prt (depths, 23, 65);
}
...
prt_state ()
{
  ...
  if (py.flags.paralysis > 1)
	...
	snprintf (tmp, sizeof (tmp), "Rest %-5d", py.flags.rest);
      else
	snprintf (tmp, sizeof (tmp), "%s", "Rest");
    ...
    {
      if (display_counts)
	snprintf (tmp, sizeof (tmp), "Repeat %-3d", command_count);
      else
	snprintf (tmp, sizeof (tmp), "%s", "Repeat");
      py.flags.status |= PY_REPEAT;
      put_buffer (tmp, 23, 38);
...
get_name ()
{
  ...
  if (!get_string (py.misc.name, 2, 15, 23) || py.misc.name[0] == 0)
    {
      user_name (py.misc.name, sizeof (py.misc.name));
     ...
     register int item_val, drop_all;
{
  ...
  if (item_val >= INVEN_WIELD)
      ...
      else
	{
      ...
      objdes (prt1, sizeof (prt1), &t_list[i], TRUE);
      snprintf (prt2, sizeof (prt2), "Dropped %s", prt1);
      msg_print (prt2);
    }
  py.flags.status |= PY_STR_WGT;
}
     ...
     char *msg;
{
  ...
  for (i = 0; i < inven_ctr; i++)
	...
	objdes (long_desc, sizeof (long_desc), &lost_obj, TRUE);
	old_ctr = inven_ctr;
	inven_destroy (i);
	j++;
	if (j == 1)
	  msg_print (msg);
	snprintf (long_msg, sizeof (long_msg), "You lost %s", long_desc);
     ...
     int spell;
{
  ...
  s_ptr = &magic_spell[py.misc.pclass][spell];
     ...
     int comment, nonconsec;
{
  ...
  if (comment)
    ...
    col = 31;
  offset =
    (class[py.misc.pclass].spell == MAGE ? 0 : 1);
  erase_line (1, col);
  ...
  for (i = 0; i < num; i++)
    {
      ...
      if (comment == FALSE)
	p = "";
      else if (spell_status[j].forgotten == 1)
	p = " forgotten";
      else if (spell_status[j].learned == 0)
	p = " unknown";
      else if (spell_status[j].worked == 0)
	p = " untried";
      else
	p = "";
	...
	spell_char = 'a' + j - nonconsec;
      snprintf (out_val, sizeof (out_val),
		"  %c) %-30s%2d %4d %3d%%%s", spell_char,
		spell_names[offset][j], s_ptr->slevel, s_ptr->smana,
		spell_chance (j), p);
      prt (out_val, 2 + i, col);
    }
}
	  ...
	  *sn = choice - 'A' + first_spell;
	  ...
	  if (i == num)
	    *sn = -2;
	    ...
	    {
	      s_ptr = &magic_spell[py.misc.pclass][*sn];
	      snprintf (tmp_str, sizeof (tmp_str),
			"Cast %s (%d mana, %d%% fail)?",
			spell_names[offset][*sn], s_ptr->smana,
			spell_chance (*sn));
     ...
     int stat;
{
  ...
  p_ptr = &py.misc;
  msp_ptr = &magic_spell[p_ptr->pclass][0];
  if (stat == A_INT)
    {
      p = "spell";
      offset = 0;
  ...
  for (i = MAX_SPELLS-1; i >= 0; i--)
    {
      if (spell_status[i].learned && msp_ptr[i].slevel > p_ptr->lev)
        {
          spell_status[i].learned = 0;
          spell_status[i].forgotten = 1;
	  snprintf (tmp_str, sizeof (tmp_str),
                    "You have forgotten the %s of %s.", p,
                    spell_names[offset][i]);
          msg_print (tmp_str);
	  }
	else
	  break;
  ...
  if (new_spells > 0)
      ...
      for (i = 0; i < MAX_SPELLS; i++)
	{
	  ...
	  if (spell_status[j].forgotten)
		...
		{
		  new_spells--;
                  spell_status[j].forgotten = 0;
                  spell_status[j].learned = 1;
		  snprintf (tmp_str, sizeof (tmp_str),
			    "You have remembered the %s of %s.", p,
			    spell_names[offset][j]);
		  msg_print (tmp_str);
		}
	      else
		num_allowed++;
      ...
      if (new_spells > 0)
	{
	  ...
	  /* must check all spells here, in gain_spell() we actually check
	     if the books are present */
          ...
          for (i = 0; i < MAX_SPELLS; i++)
            {
              if (spell_status[i].learned == 0 &&
                  msp_ptr[i].slevel <= p_ptr->lev)
                k++;
            }
	  if (new_spells > k)
	    new_spells = k;
  ...
  if (new_spells != py.flags.new_spells)
	...
	{
	  snprintf (tmp_str, sizeof (tmp_str),
		    "You can learn some new %ss now.", p);
	  msg_print (tmp_str);
	}
...
/* check if we have a given spell (by index) in a book of the given tval -BA */
int has_spell_in_book(int spell_idx, int tval)
{
  int i;
  int has_spell = 0;
  for (i = 0; i < inven_ctr; i++)
    if (inventory[i].tval == tval)
      has_spell |= object_list[inventory[i].index].spells[spell_idx];
  return has_spell;
}
...
gain_spells ()
{
  ...
  int spells[MAX_SPELLS], offset, last_known;
  register int i, j;
  ...
  if (class[p_ptr->pclass].spell == MAGE)
    ...
    {
      snprintf (tmp_str, sizeof (tmp_str), "You can't learn spells!");
      msg_print (tmp_str);
      free_turn_flag = TRUE;
      return;
    }

  for (last_known = 0; last_known < MAX_SPELLS; last_known++)
    if (spell_order[last_known] == MAX_SPELLS)
      break;
...
gain_level ()
{
  ...
  p_ptr = &py.misc;
  p_ptr->lev++;
  snprintf (out_val, sizeof (out_val), "Welcome to level %d.",
	    (int) p_ptr->lev);
  msg_print (out_val);
  calc_hitpoints ();
...
void
insert_str (object_str, object_str_len, mtc_str, insert)
     char *object_str;
     size_t object_str_len;
     char *mtc_str, *insert;
{
  int obj_len;
  char *bound, *pc;
  ...
  if (pc <= bound)
    {
      size_t len = sizeof (out_val) - 1;
      if ((pc - object_str) < len)
	len = pc - object_str;
      strncpy (out_val, object_str, len);
      out_val[(int) len] = '\0';
      char *suffix = strdup (pc + mtc_len);
      snprintf (object_str, object_str_len, "%s%s%s",
		out_val, insert != NULL ? insert : "", 
		suffix == NULL ? "" : suffix);
      free (suffix);
    }
}
...
void
insert_lnum (object_str, object_str_siz, mtc_str, number, show_sign)
     char *object_str;
     size_t object_str_siz;
  ...
  if (string)
    {
      ...
      strncpy (str2, string + mlen, sizeof (str2));
      if ((number >= 0) && (show_sign))
	snprintf (object_str, object_str_siz, "%s+%ld%s", str1, number, str2);
      else
	snprintf (object_str, object_str_siz, "%s%ld%s", str1, number, str2);
    }
}
