  ...
  if (c_ptr->cptr > 1)
    {
      ...
      if (m_ptr->ml)
	snprintf (m_name, sizeof (m_name), "The %s",
		  c_list[m_ptr->mptr].name);
      else
	snprintf (m_name, sizeof (m_name), "Something");
      snprintf (out_val, sizeof (out_val), "%s is in your way!", m_name);
      msg_print (out_val);
      ...
      else if (c_ptr->tptr != 0)
	{
	  ...
	  if (i == TV_VIS_TRAP && i_ptr->subval > 0)	/* Floor trap    */
	      ...
	      else
		{
		  ...
		  move_char (dir, FALSE);
		  py.flags.confused += tmp;
                  /* once the floor trap goes off, it's expired. 
                     it can't go off twice! */
	          t_list[c_ptr->tptr].subval = 0;
		}
	    }
	  else if (i == TV_CHEST)
	    {
	      ...
	      else if (CH_TRAPPED & i_ptr->flags)
		{
		  if ((tot - level) > randint (100))
		  ...
		  else
		    {
		      ...
		      known2 (i_ptr);
		      chest_trap (y, x);
                      /* once the floor trap goes off, it's gone.
                       it can't go off twice! */
                      i_ptr->flags &= ~CH_TRAPPED;
		    }
		}
  ...
  if (x < 0 || y < 0 || y > x)
    {
      snprintf (tmp_str, sizeof (tmp_str),
		"Illegal call to look_see(%d, %d)", x, y);
      msg_print (tmp_str);
    }
  if (x == 0 && y == 0)
    dstring = "You are on";
  ...
  if (gl_rock == 0 && c_ptr->cptr > 1 && m_list[c_ptr->cptr].ml)
    {
      ...
      snprintf (out_val, sizeof (out_val), "%s %s %s. [(r)ecall]",
		dstring,
  ...
  if (c_ptr->tl || c_ptr->pl || c_ptr->fm)
      ...
      if (c_ptr->tptr != 0)
	{
	  ...
	  if (gl_rock == 0 && t_list[c_ptr->tptr].tval != TV_INVIS_TRAP)
	    {
	      objdes (tmp_str, sizeof (tmp_str), &t_list[c_ptr->tptr], TRUE);
	      snprintf (out_val, sizeof (out_val),
			"%s %s ---pause---", dstring, tmp_str);
	      dstring = "It is in";
	      prt (out_val, 0, 0);
	      move_cursor_relative (y, x);
	      query = inkey ();
...
/* Obtain the hit and damage bonuses and the maximum distance for a
   thrown missile. */
static void
facts (i_ptr, tbth, tpth, tdam, tdis)
     register inven_type *i_ptr;
     ...
     inven_type *t_ptr;
{
  ...
  if (flag)
    ...
    {
      objdes (tmp_str, sizeof (tmp_str), t_ptr, FALSE);
      snprintf (out_val, sizeof (out_val), "The %s disappears.", tmp_str);
      msg_print (out_val);
    }
}
...
throw_object ()
{
  ...
  if (inven_ctr == 0)
    ...
    if (get_item
	(&item_val, "Fire/Throw which one?", 0, inven_ctr - 1, NULL, NULL))
    {
      if (get_dir (NULL, &dir))
	{
	  ...
	  do
	      ...
	      if ((c_ptr->fval <= MAX_OPEN_SPACE) && (!flag))
		{
		  if (c_ptr->cptr > 1)
			...
			{
			  i = m_ptr->mptr;
			  objdes (tmp_str, sizeof (tmp_str), &throw_obj,
				  FALSE);
     ...
     int y, x;
{
  ...
  /* Does the player know what he's fighting?      */
  if (!m_ptr->ml)
    snprintf (m_name, sizeof (m_name), "it");
  else
    snprintf (m_name, sizeof (snprintf), "the %s", c_ptr->name);
  ...
  if (test_hit (base_tohit, (int) py.misc.lev,
    ...
    {
      snprintf (out_val, sizeof (out_val), "You hit %s.", m_name);
      msg_print (out_val);
	...
	{
	  snprintf (out_val, sizeof (out_val), "You have slain %s.", m_name);
	  msg_print (out_val);
	  prt_experience ();
	}
      else
	{
	  ...
	  if ((100 + randint (400) + randint (400))
		...
		m_ptr->stunned = 24;
	      snprintf (out_val, sizeof (out_val),
			"%s appears stunned!", m_name);
	    }
	  else
	    snprintf (out_val, sizeof (out_val),
		      "%s ignores your bash!", m_name);
	  msg_print (out_val);
	}
    }
  else
    {
      snprintf (out_val, sizeof (out_val), "You miss %s.", m_name);
      msg_print (out_val);
    }
