  ...
  for (i = r1; i <= r2; i++)	/* Print the items       */
    {
      if (mask == NULL || mask[i])
	{
	  objdes (tmp_val, sizeof (tmp_val), &inventory[i], TRUE);
	  tmp_val[lim] = 0;	/* Truncate if too long. */
	  snprintf (out_val[i], sizeof (out_val[i]), "%c) %s",
		    'a' + i, tmp_val);
	  ...
	  if (weight)
	    {
	      ...
	      snprintf (tmp_val, sizeof (tmp_val), "%3d.%d lb",
     ...
     int weight, col;
{
  ...
  for (i = INVEN_WIELD; i < INVEN_ARRAY_SIZE; i++)	/* Range of equipment */
      ...
      if (i_ptr->tval != TV_NOTHING)
	{
	  ...
	  snprintf (out_val[line], sizeof (out_val[line]),
		    "%c) %-14s: %s", line + 'a', prt1, prt2);
	  ...
	  if (weight)
	    {
	      ...
	      snprintf (prt2, sizeof (prt2), "%3d.%d lb",
			(total_weight) / 10, (total_weight) % 10);
	      prt (prt2, line + 1, 71);
	    }
	  line++;
     ...
     int item_val, posn;
{
  ...
  objdes (prt2, sizeof (prt2), t_ptr, TRUE);
  if (posn >= 0)
    snprintf (out_val, sizeof (out_val), "%s%s (%c)", p, prt2, 'a' + posn);
  else
    snprintf (out_val, sizeof (out_val), "%s%s", p, prt2);
  msg_print (out_val);
     ...
     int item;
{
  ...
  objdes (object, sizeof (object), &inventory[item], TRUE);
  object[strlen (object) - 1] = '?';	/* change the period to a question mark */
  snprintf (out_str, sizeof (out_str), "%s %s", prompt, object);
  ...
  do
	...
	case 'w':		/* Wear/wield    */
	  ...
	  for (wear_high = wear_low;
	       wear_high < inven_ctr
	       && inventory[wear_high].tval >= TV_MIN_WEAR; wear_high++)
	    ;
	  wear_high--;
	...
	case 'x':
	  if (inventory[INVEN_WIELD].tval == TV_NOTHING &&
	    ...
	    {
	      objdes (prt1, sizeof (prt1), &inventory[INVEN_WIELD], FALSE);
	      snprintf (prt2, sizeof (prt2),
		...
		{
		  bigvtype out_val;
		  snprintf (prt1, sizeof (prt1), "%s", "Primary weapon   : ");
		  objdes (prt2, sizeof (prt2), &inventory[INVEN_WIELD], TRUE);
		  snprintf (out_val, sizeof (out_val), "%s%s", prt1, prt2);
		  msg_print (out_val);
		}
	      else
      ...
      while (selecting && free_turn_flag)
	{
	  ...
	  if (from > to)
		...
		disp = "";
	      snprintf (prt1, sizeof (prt1),
			"(%c-%c%s%s%s, space to break, "
			"ESC to exit) %s which one?",
			from + 'a', to + 'a', disp, swap,
			((command == 'w'
			  || command == 'd') ? ", 0-9" : ""), prompt);
	      /* Abort everything. */
		  ...
		  if (item < from || item > to)
		      ...
		      else if (command == 'w')
			{
			  ...
			  if (isupper ((int) which) && !verify (prompt, item))
				...
				if (inventory[INVEN_RIGHT].tval == TV_NOTHING)
				  slot = INVEN_RIGHT;
				else if (inventory[INVEN_LEFT].tval ==
					 TV_NOTHING)
				  slot = INVEN_LEFT;
				else
				  {
				    ...
				    do
				      {
					if (!get_com
					    ("Put ring on which hand (l/r/L/R)?",
					     &query))
					  {
					    item = -1;
					    slot = -1;
				...
				break;
			      default:
				msg_print
				  ("IMPOSSIBLE: I don't see how you can use that.");
				item = -1;
				break;
			      }
			  if (item >= 0 && inventory[slot].tval != TV_NOTHING)
				...
				{
				  objdes (prt1, sizeof (prt1),
					  &inventory[slot], FALSE);
				  snprintf (prt2, sizeof (prt2),
					    "The %s you are %s%s",
					    prt1, slot == INVEN_HEAD ?
					    "wielding " : "wearing ",
					    "appears to be cursed.");
				  msg_print (prt2);
				  item = -1;
				}
			      else if (inventory[item].subval ==
				       ITEM_GROUP_MIN
			  ...
			  if (item >= 0)
				...
				string = "You are wearing";
			      objdes (prt2, sizeof (prt2), i_ptr, TRUE);
		      ...
		      else	/* command == 'd' */
			{
			  if (inventory[item].number > 1)
			    {
			      objdes (prt1, sizeof (prt1), &inventory[item],
				      TRUE);
			      prt1[strlen (prt1) - 1] = '?';
			      snprintf (prt2, sizeof (prt2),
					"Drop all %s [y/n]", prt1);
      ...
      else
	{
	  ...
	  if (scr_state == INVEN_SCR)
		...
		snprintf (prt1, sizeof (prt1),
			  "You are carrying %d.%d pounds. "
			  "In your pack there is %s",
			  inven_weight / 10, inven_weight % 10,
			  (inven_ctr == 0 ? "nothing." : "-"));
	      else
		snprintf (prt1, sizeof (prt1),
			  "You are carrying %d.%d pounds. "
			  "Your capacity is %d.%d pounds. %s",
			  inven_weight / 10, inven_weight % 10,
...
int
get_item (com_val, pmt, i, j, mask, message)
     int *com_val;
     char *pmt;
     int i, j;
     char *mask;
		...
		case '/':
		  if (full)
		      ...
		      if (i_scr > 0)
			{
			  if (equip_ctr == 0)
			    {
			      prt ("But you're not using anything -more-", 0,
				   0);
			      inkey ();
			    }
			  else
		      ...
		      else
			{
			  if (inven_ctr == 0)
			    {
			      prt ("But you're not carrying anything -more-",
				   0, 0);
			      inkey ();
			    }
			  else
     ...
     int8u deflt;
{
  ...
  if ((max_num = item->number) == 1 || item->subval > ITEM_GROUP_MIN)
    return (max_num);
  snprintf (pmt_str, sizeof (pmt_str), "(0-%d, ESC to exit) %s [%d] ",
	    max_num, pmt, deflt);
  col = strlen (pmt_str);
  prt (pmt_str, 0, 0);
     ...
     char *hit_from;
{
  ...
  if (py.misc.chp < 0)
	...
	{
	  death = TRUE;
	  snprintf (died_from, sizeof (died_from), "%s", hit_from);
	  total_winner = FALSE;
	}
      new_level_flag = TRUE;
    }
