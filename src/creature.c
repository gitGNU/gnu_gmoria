     ...
     int monptr;
{
  ...
  r_ptr = &c_list[m_ptr->mptr];
  if (!m_ptr->ml)
    snprintf (cdesc, sizeof (cdesc), "%s", "It ");
  else
    snprintf (cdesc, sizeof (cdesc), "The %s ", r_ptr->name);
  /* For "DIED_FROM" string        */
  if (CM_WIN & r_ptr->cmove)
    snprintf (ddesc, sizeof (ddesc), "The %s", r_ptr->name);
  else if (is_a_vowel (r_ptr->name[0]))
    snprintf (ddesc, sizeof (ddesc), "an %s", r_ptr->name);
  else
    snprintf (ddesc, sizeof (ddesc), "a %s", r_ptr->name);
  /* End DIED_FROM                 */
  ...
  while ((*attstr != 0) && !death)
	...
	case 23:		/*Eat light         */
	  ...
	  break;
	case 24:		/*Eat charges       */
	  if ((test_hit (15, (int) r_ptr->level, 0, p_ptr->pac + p_ptr->ptoac, CLA_MISC_HIT)) && (inven_ctr > 0))	/* check to make sure an object exists */
	    flag = TRUE;
	  break;
	case 99:
	  flag = TRUE;
	...
	default:
      ...
      if (flag)
	{
	  /* can not concatendate to cdesc because the creature may have
	     multiple attacks */
	  disturb (1, 0);
	  tmp_str[0] = '\0';
	  switch (adesc)
	    {
	    case 1:
	      snprintf (tmp_str, sizeof (tmp_str), "%s%s", cdesc,
			"hits you.");
	      break;
	    case 2:
	      snprintf (tmp_str, sizeof (tmp_str), "%s%s", cdesc,
	  ...
	  switch (attype)
	      ...
	      if (flag)
		{
		  ...
		  objdes (long_desc, sizeof (long_desc), i_ptr, FALSE);
		  snprintf (long_msg, sizeof (long_msg),
			    "Your %s was disenchanted!", long_desc);
		  msg_print (long_msg);
		  calc_bonuses ();
		}
	      else
	  ...
	  if (py.flags.confuse_monster && adesc != 99)
		...
		snprintf (tmp_str, sizeof (tmp_str),
			  "%sis unaffected.", cdesc);
	      else
		{
		  snprintf (tmp_str, sizeof (tmp_str),
			    "%sappears confused.", cdesc);
		  if (m_ptr->confused)
		    m_ptr->confused += 3;
		  else
     ...
     int *took_turn;
{
  ...
  /* Must have unobstructed Line-Of-Sight          */
    ...
    *took_turn = FALSE;
	...
	{
	  snprintf (outval, sizeof (outval), "%s%s", cdesc, "casts a spell.");
	  msg_print (outval);
	}
      /* Cast the spell.                             */
      switch (thrown_spell)
	...
	case 14:		/*Summon Monster */
	  snprintf (outval, sizeof (outval), "%s%s", cdesc,
	...
	case 17:		/*Drain Mana   */
	  if (py.misc.cmana > 0)
	    {
	      disturb (1, 0);
	      snprintf (outval, sizeof (outval),
			"%sdraws psychic energy from you!", cdesc);
	      msg_print (outval);
	      if (m_ptr->ml)
		{
		  snprintf (outval, sizeof (outval),
			    "%sappears healthier.", cdesc);
		  msg_print (outval);
		}
	...
	case 20:		/*Breath Light */
	  ...
	  break;
	case 21:		/*Breath Gas   */
	  snprintf (outval, sizeof (outval), "%s%s", cdesc, "breathes gas.");
	  msg_print (outval);
	  breath (GF_POISON_GAS, char_row, char_col, (m_ptr->hp / 3), ddesc,
		  monptr);
	  break;
	case 22:		/*Breath Acid  */
	  snprintf (outval, sizeof (outval), "%s%s", cdesc, "breathes acid.");
	  msg_print (outval);
	  breath (GF_ACID, char_row, char_col, (m_ptr->hp / 3), ddesc,
		  monptr);
	  break;
	case 23:		/*Breath Frost */
	  snprintf (outval, sizeof (outval), "%s%s", cdesc,
		    "breathes frost.");
	  msg_print (outval);
	  breath (GF_FROST, char_row, char_col, (m_ptr->hp / 3), ddesc,
		  monptr);
	  break;
	case 24:		/*Breath Fire  */
	  snprintf (outval, sizeof (outval), "%s%s", cdesc, "breathes fire.");
	  msg_print (outval);
	  breath (GF_FIRE, char_row, char_col, (m_ptr->hp / 3), ddesc,
		  monptr);
	  break;
	default:
	  snprintf (outval, sizeof (outval), "%s%s", cdesc,
		    "casts an unknown spell that has absolutely no effect.");
	  msg_print (outval);
	}
      /* End of spells                                 */
      if (m_ptr->ml)
      ...
      if (cave[m_ptr->fy][m_ptr->fx].fval >= MIN_CAVE_WALL)
	{
	  ...
	  if (i >= 0)
	  ...
	  else
	    {
	      ...
	      twall ((int) m_ptr->fy, (int) m_ptr->fx, 1, 0);
	    }
	}
		...
		if (m_ptr->ml || (m_ptr->cdis <= c_list[m_ptr->mptr].aaf)
			...
			&& cave[m_ptr->fy][m_ptr->fx].fval >= MIN_CAVE_WALL))
		  {
			...
			if (py.flags.aggravate)
			  ...
			  if ((py.flags.rest == 0 && py.flags.paralysis < 1)
			      || (randint (50) == 1))
			  {
			    notice = randint (1024);
			...
			if (m_ptr->stunned == 0)
			  {
			    if (m_ptr->ml)
			      {
				snprintf (cdesc, sizeof (cdesc), "The %s%s",
