/* st_symtab.c: routines for managing symbol tables
 *
...
void
oldSt_SArgzTable (st_Table_Pt table, char **argz, size_t *argz_len)
{
  char **s_A, **sp;
  int type, i, count = 0;
  generic_t gval;

  s_A = St_SListTable (table);

  /* we go backwards here so that it'll appear in order. grr*/
  for (sp = s_A; *sp; sp++)
    count++;
  sp--;
  for (i = 0; i < count; i++)
    {
      argz_add (argz, argz_len, *sp);
      sp--;
    }
...
void
St_SArgzTable (st_Table_Pt table, char **argz, size_t *argz_len)
{
  char **s_A, **sp;
  int type;
  generic_t gval;
